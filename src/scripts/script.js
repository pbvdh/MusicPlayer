...
const APP = (function () {
  ...
  async function init() {
    ...
    //add event listeners for clicking into playlists using name
    addPlaylistEventListeners();

    //add event listeners for the kebab menu on song cards
    addActionMenuEventListeners(actionButtons);

    //add event listeners for song name or artist name to play or search respectively
    ...
    previousTrackButton.addEventListener('click', async function () {
      if (queueList.childElementCount > 0) {
        ...
        if (nowPlayingInfo.currentSongIndex > 0) {
          ...
          //restart song
          await playSong(nowPlayingInfo.queue[nowPlayingInfo.currentSongIndex]);
        }
      }
    });
    ...
    nextTrackButton.addEventListener('click', async function () {
      if (queueList.childElementCount > 0) {
        let currentTrack = queueList.querySelector(".queuecurrent").querySelector(".songname");
        if (nowPlayingInfo.currentSongIndex + 1 < nowPlayingInfo.queue.length) {
          nowPlayingInfo.currentSongIndex++;
          playSong(nowPlayingInfo.queue[nowPlayingInfo.currentSongIndex]);
    ...
    //shuffle library
    shuffleLibraryButton.addEventListener('click', function () {
      //empty queue and disable shuff;le
      queueList.innerHTML = "";
      nowPlayingInfo.queue = [];
      nowPlayingInfo.currentSongIndex = 0;
      //get random song name
      let list = songList.querySelectorAll('li:not([style="display: none;"])');
      let songName = list[Math.floor(Math.random() * list.length)].querySelector(".songname");
  ...
  //ad click event listeners for song and artist links 
  ...
  function searchSongByName(searchBar, ul, includeSongs = true) {
    // Declare variables
    ...
    for (i = 0; i < li.length; i++) {
      ...
      if ((songValue.toLowerCase().indexOf(filter) > -1 && includeSongs) || artistValue.toLowerCase().indexOf(filter) > -1) {
        li[i].style.display = "";
      } else {
  ...
  function searchLinkedTrack(searchBar, a) {
    ...
    if (a.classList.contains("artistname")) {
      //we are filtering by artists with this exact name, we dont want songs that match as we are searching for an artist
      searchSongByName(searchBar, songList, false);
      searchSongByName(searchBar, artistList);
    } else {
      searchSongByName(searchBar, songList);
      searchSongByName(searchBar, artistList);
    ...
    switch (option.toLowerCase()) {
      ...
      case "remove from queue":
        ...
        if (songCard.querySelector(".tracklistitem").classList.contains("queuecurrent")) {
          ...
          songCard.remove();
          if (selectedSongIndex < nowPlayingInfo.currentSongIndex) {
            nowPlayingInfo.currentSongIndex--;
          }
          updateQueueAppearance(currentSong, false);
        }
                ...
                <div class="tracklistitem">
                  <div class="trackdetailscontainer">
                    <div class="overflowwrapper">
                      <a class="songname" draggable=false songid ="${song.id}" title="${song.name}">${song.name}</a>
                      <br>
      ...
      listItems +=
        `<li>
                <div class="tracklistitem">
                  <div class="trackdetailscontainer">
                    <div class="overflowwrapper">
                      <a class="artistname songsearchlink" title="${artist.artist_name}">${artist.artist_name}</a>
                    </div>
                  </div>
                </div>
              </li>`;
      ...
      listItems +=
        `<li>
          <div class="tracklistitem">
            <div class="trackdetailscontainer">
              <div class="overflowwrapper">
                <a class="playlistname" playlistid="${playlist.id}" title="${playlist.name}">${playlist.name}</a>
  ...
  //generate playlist cards by querying playlists from database
  ...
  //form HTML for an action menu depending on what type of card it is for
  ...
  //when hovering over add to playlist, display list of playlists
  ...
  //update the position of a song in playlist in the database
